...
export default function EventDetailPage() {
  ...
  if (event) {
    if (event.images && event.images.length > 0) {
      ...
      if (event.imageData) allImages.push(event.imageData);
    }
  }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontFamily: MONO, fontSize: 12, color: '#9ca3af', letterSpacing: '0.1em' }}>LOADING...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff' }}>
      ...
      <div style={{ position: 'relative', height: '60vh', minHeight: 360, overflow: 'hidden' }}>
        ...
        {/* Nav */}
        <div style={{ position: 'absolute', top: 24, left: 40, display: 'flex', alignItems: 'center', gap: 8, zIndex: 2 }}>
        ...
        {/* Bottom info */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '40px 60px', zIndex: 2 }}>
          <div style={{ marginBottom: 12 }}>
            <Badge variant={event.status} />
          </div>
          <h1 style={{ fontFamily: BEBAS, fontSize: 'clamp(32px,5vw,60px)', color: '#fff', letterSpacing: '0.02em', lineHeight: 1, marginBottom: 14, wordBreak: 'keep-all' }}>
            {event.title}
          </h1>
          <div style={{ display: 'flex', gap: 20, fontSize: 13, color: 'rgba(255,255,255,0.75)', fontFamily: MONO, letterSpacing: '0.04em', flexWrap: 'wrap' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '56px 40px 80px' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
          ...
          {/* Benefit badge */}
          {event.benefit && (
            <div style={{ display: 'inline-block', background: '#f0fdf4', border: '1px solid rgba(22,163,74,0.3)', borderRadius: 9999, padding: '6px 16px', fontSize: 12, color: '#16a34a', fontWeight: 700, marginBottom: 28 }}>
              {event.benefit}
            </div>
          )}
          ...
          {/* Description */}
          {event.desc && (
            <p style={{ fontSize: 17, color: '#1f2937', lineHeight: 1.85, marginBottom: 32, whiteSpace: 'pre-wrap', letterSpacing: '-0.01em' }}>
              {event.desc}
            </p>
          )}

          {/* Content (rich text / detailed) */}
          {event.content && event.content !== event.desc && (
            <div style={{ marginBottom: 40, paddingTop: 32, borderTop: '1px solid #f0f0f0' }}>
              <p style={{ fontSize: 15, color: '#374151', lineHeight: 1.85, whiteSpace: 'pre-wrap' }}>
                {event.content}
              </p>
            </div>
          )}
          ...
          {allImages.length > 0 && (
            <div style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <h2 style={{ fontFamily: BEBAS, fontSize: 26, color: '#111', letterSpacing: '0.02em', lineHeight: 1 }}>갤러리</h2>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', paddingTop: 8 }}>
            {event.link ? (
              <Link href={event.link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', padding: '13px 30px', borderRadius: 9999, background: '#16a34a', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
                {event.ctaText || '신청하기 →'}
              </Link>
            ) : (
              <span style={{ display: 'inline-flex', padding: '13px 30px', borderRadius: 9999, background: '#f3f4f6', color: '#9ca3af', fontSize: 14, fontWeight: 700 }}>
                {event.ctaText || '준비 중'}
              </span>
            )}
            <Link href="/events" style={{ display: 'inline-flex', padding: '13px 30px', borderRadius: 9999, border: '1.5px solid #e5e7eb', color: '#374151', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
              ← 목록으로
            </Link>
          </div>
        </motion.div>
      ...
      <AnimatePresence>
        {lightboxIdx !== null && (
          ...
          >
            {/* Close */}
            <button onClick={closeLightbox} style={{ position: 'absolute', top: 20, right: 24, color: '#fff', fontSize: 28, background: 'none', border: 'none', cursor: 'pointer', lineHeight: 1, opacity: 0.7 }}>✕</button>
