...
export default function AboutContent() {
  return (
    <>
      {/* Vision */}
      <section id="vision" style={{ padding: '88px 60px', borderBottom: '1px solid #e5e7eb', background: '#f8fafb' }}>
        <motion.div
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
            {VALUES.map((v, i) => (
              <motion.div
                ...
                variants={fadeUp}
                custom={i * 0.06}
                style={{ background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 14, padding: '20px 22px' }}
              >
                ...
                <p style={{ fontSize: 13, color: '#111', fontWeight: 500, lineHeight: 1.5 }}>{v.text}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {[
              ...
              <motion.a
                ...
                variants={fadeUp}
                custom={i * 0.06}
                whileHover={{ y: -10, boxShadow: '0 16px 40px rgba(0,0,0,0.1)' }}
              ...
              >
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <h3 style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 22, color: '#111', letterSpacing: '0.02em', marginBottom: 10 }}>
                    {item.title}
                  </h3>
                  <p style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.75, fontWeight: 300 }}>
                    {item.desc}
                  </p>
                  <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: item.color }}>자세히 보기</span>
                    <span style={{ fontSize: 14, color: item.color }}>→</span>
                  </div>
                </div>
              </motion.a>
            ))}
      ...
      {/* Stats */}
      <section style={{ padding: '64px 60px', borderBottom: '1px solid #e5e7eb', background: '#fff' }}>
        <motion.div
          variants={staggerContainer}
          initial="hidden"
          whileInView="visible"
        ...
        >
          {[
            ...
            <motion.div
              ...
              variants={fadeUp}
              custom={i * 0.06}
              style={{ padding: '40px 0', borderRight: i < 2 ? '1px solid #e5e7eb' : 'none' }}
            >
              <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 'clamp(36px, 5vw, 56px)', color: '#16a34a', lineHeight: 1, marginBottom: 8 }}>
                <CountUp value={s.val} suffix={s.suf} />
              </div>
              <div style={{ fontFamily: "'SF Mono', 'Menlo', 'Monaco', 'Consolas', 'Courier New', monospace", fontSize: 10, color: '#6b7280', letterSpacing: '0.1em' }}>{s.label}</div>
            </motion.div>
          ))}
        </motion.div>
      </section>
