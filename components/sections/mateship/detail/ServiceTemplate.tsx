...
export default function ServiceTemplate({ partner }: { partner: MateshipPartner }) {
  const { detail, name, icon, gradient, region, type, discount } = partner;
  if (!detail) return null;

  return (
    <main>
      {/* Hero */}
      <section style={{ padding: '72px 60px 56px', textAlign: 'center', background: '#fafafa', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{
          ...
          MATESHIP PARTNER · {type.toUpperCase()}
        </div>
        ...
        {region !== '-' && (
          <p style={{ fontFamily: MONO, fontSize: 12, color: '#6b7280', letterSpacing: '0.04em', marginBottom: 12 }}>
            📍 {region}
          </p>
        )}
      ...
      <section style={{ padding: '64px 60px', background: '#fff', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{
            ...
            <p style={{ fontFamily: MONO, fontSize: 12, color: '#16a34a', letterSpacing: '0.06em' }}>MEMBERS ONLY</p>
            <p style={{ fontFamily: BEBAS, fontSize: 36, color: '#16a34a', letterSpacing: '0.04em' }}>✨ {discount}</p>
          </div>
        </div>
      </section>
      ...
      {detail.howToUse && detail.howToUse.length > 0 && (
        <section style={{ padding: '64px 60px', background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
      ...
      {detail.infoSections && detail.infoSections.length > 0 && (
        <section style={{ padding: '48px 60px', background: '#fff', borderBottom: '1px solid #e5e7eb' }}>
          <div style={{ maxWidth: 600, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: BEBAS, fontSize: 'clamp(28px,4vw,44px)', color: '#111', marginBottom: 24, letterSpacing: '0.02em' }}>
              이용 안내
            </h2>
            {detail.infoSections.map((sec, i) => (
              <div key={i} style={{ marginBottom: 24, padding: '20px', borderRadius: 12, background: '#f9fafb', border: '1px solid #e5e7eb' }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: '#111', marginBottom: 10 }}>{sec.title}</h3>
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {sec.items.map((item, j) => (
                    <li key={j} style={{ fontSize: 13, color: '#6b7280', display: 'flex', gap: 8, lineHeight: 1.6 }}>
      ...
      <section style={{ padding: '64px 60px', background: '#111', textAlign: 'center' }}>
        <h2 style={{ fontFamily: BEBAS, fontSize: 'clamp(24px,3.5vw,36px)', color: '#fff', marginBottom: 8, letterSpacing: '0.02em' }}>
          메이트쉽 회원이라면 지금 바로 혜택을
        </h2>
        <p style={{ fontSize: 14, color: '#6b7280', marginBottom: 32 }}>
          메이트쉽 디지털 회원증(QR) 제시 후 바로 할인 적용됩니다.
        </p>
        {detail.cta && (
          <a
            href={detail.cta.url}
